{pkgs, ...}: {
  services.udev = {
    ...
    extraRules = ''
      #SUBSYSTEM=="usb", ACTION=="remove", ENV{PRODUCT}=="17ef/100f/1", RUN+="${pkgs.systemd}/bin/systemctl start dock_handler.service"
      #SUBSYSTEM=="usb", ACTION=="add", ENV{PRODUCT}=="17ef/100f/1", RUN+="${pkgs.bash}/bin/bash /home/hochi/a.sh"

      #ACTION=="add", SUBSYSTEM=="backlight", KERNEL=="intel_backlight", RUN+="${pkgs.busybox}/bin/chgrp wheel /sys/class/backlight/%k/brightness"
      #ACTION=="add", SUBSYSTEM=="backlight", KERNEL=="intel_backlight", RUN+="${pkgs.busybox}/bin/chmod g+w /sys/class/backlight/%k/brightness"

      SUBSYSTEM=="power_supply", ATTR{online}=="0", RUN+="${pkgs.systemd}/bin/systemctl stop syncthing.service"
      SUBSYSTEM=="power_supply", ATTR{online}=="1", RUN+="${pkgs.systemd}/bin/systemctl start syncthing.service"
      ...
      ATTRS{idVendor}=="0ce9", MODE="0666", ENV{ID_MM_DEVICE_IGNORE}="1", ENV{ID_MM_PORT_IGNORE}="1"

      # Saleae Logic
      ATTR{idVendor}=="0925", ATTR{idProduct}=="3881", MODE="0666", ENV{ID_MM_DEVICE_IGNORE}="1", ENV{ID_MM_PORT_IGNORE}="1"
      ATTR{idVendor}=="21a9", ATTR{idProduct}=="1001", MODE="0666", ENV{ID_MM_DEVICE_IGNORE}="1", ENV{ID_MM_PORT_IGNORE}="1"
      ATTR{idVendor}=="21a9", ATTR{idProduct}=="1003", MODE="0666", ENV{ID_MM_DEVICE_IGNORE}="1", ENV{ID_MM_PORT_IGNORE}="1"
      ATTR{idVendor}=="21a9", ATTR{idProduct}=="1004", MODE="0666", ENV{ID_MM_DEVICE_IGNORE}="1", ENV{ID_MM_PORT_IGNORE}="1"
      ATTR{idVendor}=="21a9", ATTR{idProduct}=="1005", MODE="0666", ENV{ID_MM_DEVICE_IGNORE}="1", ENV{ID_MM_PORT_IGNORE}="1"
      ATTR{idVendor}=="21a9", ATTR{idProduct}=="1006", MODE="0666", ENV{ID_MM_DEVICE_IGNORE}="1", ENV{ID_MM_PORT_IGNORE}="1"
    '';
  };
}
